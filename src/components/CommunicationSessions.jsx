...
export default function CommunicationSessions({ roomId, userId, myChars, theme, open, onClose }) {
  ...
  const createSession = async () => {
    ...
    const { data: recordMessage } = await supabase
      .from('messages')
      .insert({
        room_id: roomId,
        user_id: userId,
  ...
  const updateSession = async (session, status) => {
    ...
    const statusLabel = status === 'active' ? (session.kind === 'call' ? '음성 통화 중' : '문자 대화 중') : status === 'declined' ? '거절된 통화' : status === 'left' ? '입장하지 않은 문자' : `${session.kind === 'call' ? '통화' : '문자'} 종료`
  ...
  const autoVisible = activeSession && hiddenSessionId !== activeSession.id && (activeSession.status === 'active' || (activeSession.status === 'ringing' && isReceiver(activeSession)))
