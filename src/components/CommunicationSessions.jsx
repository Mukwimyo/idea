  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 120, background: `${t.bg}f2`, display: 'flex', flexDirection: 'column', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 14 }}>
        ...
        {history.length > 0 && <div style={{ marginTop: 20, display: 'grid', gap: 8 }}>
          {history.map(session => <details key={session.id} style={{ padding: 12, borderRadius: 12, background: t.panel, border: `1px solid ${t.border}`, color: t.theirText }}>
            <summary>{sessionTitle(session)} · {session.status === 'declined' ? '거절된 통화' : session.status === 'left' ? '입장하지 않은 문자' : `${session.kind === 'call' ? '통화' : '문자'} 종료`}</summary>
