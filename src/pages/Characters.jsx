...
export default function Characters() {
    ...
    const handleEditImageChange = (e) => {
        ...
        if (!file) return
        const reader = new FileReader()
        reader.onload = () => { setEditCropSrc(reader.result); setEditShowCropper(true) }
        reader.readAsDataURL(file)
    }
    ...
    const getCroppedBlob = (imageSrc, pixelCrop) => new Promise((resolve) => {
            ...
            {showCropper && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                        <button onClick={confirmCrop} style={{ background: t.point, border: 'none', borderRadius: 8, padding: '8px 20px', color: '#fff', cursor: 'pointer' }}>확인</button>
                        <button onClick={() => setShowCropper(false)} style={{ background: 'none', border: `1px solid ${t.border}`, borderRadius: 8, padding: '8px 20px', color: t.subText, cursor: 'pointer' }}>취소</button>
                    </div>
                </div>
            )}
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                    <div style={{ position: 'relative', width: 300, height: 300 }}>
                        <Cropper
                            ...
                            onCropComplete={(_, p) => setEditCroppedAreaPixels(p)}
                        />
                    </div>
                    <input type="range" min={1} max={3} step={0.1} value={editZoom} onChange={e => setEditZoom(Number(e.target.value))} style={{ width: 200 }} />
                    <div style={{ display: 'flex', gap: 10 }}>
                        <button onClick={confirmEditCrop} style={{ background: t.point, border: 'none', borderRadius: 8, padding: '8px 20px', color: '#fff', cursor: 'pointer' }}>확인</button>
                        <button onClick={() => setEditShowCropper(false)} style={{ background: 'none', border: `1px solid ${t.border}`, borderRadius: 8, padding: '8px 20px', color: t.subText, cursor: 'pointer' }}>취소</button>
                    </div>
                </div>
            )}
