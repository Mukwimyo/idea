            ...
            <div style={{ maxWidth: 400, margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20, paddingTop: 8 }}>
                    <button onClick={() => navigate(-1)} style={{
                        background: 'none', border: 'none', color: t.subText, fontSize: 24, cursor: 'pointer', padding: '0 4px', marginRight: 8
                    }}>‹</button>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}>
                            <label style={{ cursor: 'pointer' }}>
                                <div style={{
                                    ...
                                    background: imagePreview ? 'transparent' : t.bg,
                                    ...
                                    {imagePreview
                                        ...
                                        : <span style={{ fontSize: 24, color: t.border }}>+</span>
                                    }
                                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    {chars.map(c => (
                        <div key={c.id} style={{ background: t.panel, borderRadius: 12, padding: '13px 15px', display: 'flex', alignItems: 'center', gap: 12, border: `0.5px solid ${t.border}` }}>
                            ...
                            <button onClick={() => deleteChar(c.id)} style={{ background: 'none', border: 'none', color: t.subText, fontSize: 16, cursor: 'pointer', padding: 4, opacity: 0.4 }}>×</button>
                        </div>
                    ))}
                </div>
                    ...
                    <div style={{ marginTop: 16 }}>
                        ...
                        {archivedChars.map(c => (
                            <div key={c.id} style={{ background: t.bg, borderRadius: 12, padding: '12px 15px', display: 'flex', alignItems: 'center', gap: 12, border: `0.5px solid ${t.border}`, marginBottom: 8, opacity: 0.6 }}>
                                ...
                                <button onClick={() => restoreChar(c.id)} style={{ background: 'none', border: `0.5px solid ${t.border}`, borderRadius: 8, padding: '4px 10px', color: t.subText, fontSize: 11, cursor: 'pointer' }}>복원</button>
                            </div>
                        ))}
                    </div>
                )}
