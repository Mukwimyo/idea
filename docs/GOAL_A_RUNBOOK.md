...
사용자 승인 후 12:26(KST)까지 다음 작업을 수행했다.

- `%LOCALAPPDATA%\IDEA\backups\goal-a-pre-20260915\public-before-001-002.dump.dpapi`에 운영 `public` 스키마와 데이터의 백업을 저장했다.
  - 평문 아카이브는 디스크에 쓰지 않고 메모리에서 DPAPI `CurrentUser`로 암호화했다.
  - 생성 도구는 PostgreSQL `pg_dump 17.11`, DPAPI 선택적 엔트로피는 `IDEA-GOAL-A-BACKUP-V1`이다.
  - 복호화 왕복 후 `PGDMP` 헤더와 원본 SHA-256 일치를 확인했다.
  - 암호화 파일: 405,094바이트
  - 암호화 파일 SHA-256: `8A4AA66578AA76DA2CD9D8BEE4CB8F017732512BB72BF3F9EDC0DC7161FF9B27`
  - Windows용 `pg_restore`가 메모리 파이프 입력을 지원하지 않아 목록 출력 검사는 생략했다. 복원 시에는 별도 승인 아래 안전한 임시 복호화 절차가 필요하다.
- `202609140001_message_delivery_and_read_cursors.sql`을 `ON_ERROR_STOP`과 단일 트랜잭션으로 적용했다.
  - 기존 메시지 4,324건을 보존하고 순번 1~4,324를 유일하게 부여했다.
  - 읽음 커서 29건을 변환했다.
  - NULL 순번, 중복 순번, 잘못된 커서 멤버십, 커서-메시지 불일치, 무효 인덱스는 모두 0건이었다.
- `202609140002_room_summaries.sql`을 별도 단일 트랜잭션으로 적용했다.
  - 배포 전 구버전 앱의 `read_by` 갱신을 현재 로그인 사용자 본인의 단조 증가 커서로 변환하는 임시 호환 트리거를 추가했다.
  - `room_read_cursors`를 `supabase_realtime`에 등록하고 UPDATE 행 전체가 전달되도록 replica identity를 `FULL`로 설정했다.
- RLS 전 단계 DB 계약 13개가 모두 통과했다.
- 실제 운영 방의 두 구성원을 사용한 `goal_a_pre_rls_smoke.sql`이 통과했다.
  - 같은 클라이언트 ID 재전송이 한 메시지로 수렴했다.
  - 읽음 커서가 최신 메시지로 전진한 뒤 과거 메시지를 읽어도 뒤로 가지 않았다.
  - 비회원 전송·읽음 갱신은 거부됐고 허용된 방은 요약 RPC에 나타났다.
  - 전체 검사는 롤백됐으며 메시지 4,324건, 커서 29건, 테스트 메시지 0건으로 운영 행 수가 유지됐다.

### 실제 두 계정 Realtime 검증

사용자 승인 후 운영 프로젝트에 실행 ID `20260915033644-df7f694a`로 식별되는 전용 테스트 계정 2개, 프로필 2개, 공동 방 1개와 구성원 2명을 만들었다. 임시 비밀번호와 서비스 역할 키는 파일이나 출력에 남기지 않았다.

- 연결 상태에서 40개, 연결을 제거한 상태에서 20개, 새 채널로 재접속한 뒤 40개 클라이언트 ID를 전송했다.
- 각 ID를 동시에 두 번 RPC 호출했지만 저장된 서버 행은 정확히 100개였고 중복은 0개였다.
- 첫 연결에서는 채널 제거 전 5개, 재접속 후에는 40개 Realtime 이벤트를 받았다.
- Realtime으로 받지 못한 55개를 최종 조회가 모두 보정해 100개 행으로 수렴했다. 연결 해제 구간 20개가 자동 재생되지 않는 것도 확인했다.
- 수신자 커서를 최신 메시지까지 전진시킨 뒤 가장 오래된 테스트 메시지로 다시 갱신해도 커서가 뒤로 가지 않았다.
- 발신자 계정에서 수신자의 최종 커서를 조회할 수 있었다.
- 최초 SQL 등록 후에는 `room_read_cursors`의 Realtime 이벤트가 전달되지 않았다. 대시보드에도 발행 대상으로 표시되는 것을 확인한 뒤 해당 테이블의 replication을 해제·재활성화해 Realtime 등록을 새로 고쳤다.
- 재등록 후 인증된 발신자 계정이 수신자 커서의 UPDATE 이벤트를 30초 제한 안에 수신했고, 이벤트 순번과 최종 DB 순번이 모두 `4482`로 일치했다.
- 사후 집계는 테스트 계정 2개, 프로필 2개, 방 1개, 구성원 2개, 메시지 100개, 커서 1개였다.
- 운영 전체 메시지는 4,424개이며 NULL·중복 순번과 중복 `(user_id, client_message_id)`는 모두 0개다.
- `messages`는 계속 `supabase_realtime` publication에 포함되어 있다.

테스트 픽스처는 `[TEST] Goal A Realtime`과 실행 ID로 명확히 표시해 두었고, 승인받은 검증 기록으로 운영에 남겨 두었다. 앱 배포와 `202609140003` 적용은 2026-09-16 전체 운영 단계 승인 후 완료했으며 아래에 기록한다.

2026-09-16 전체 운영 단계 승인 후 RLS 경계 검사용 비공개 방 `fd4b5160-3afe-412c-8f01-5eafc4558a0c`을 같은 실행 ID로 추가했다. 발신자만 구성원이며 수신자는 구성원이 아님을 준비 스크립트가 확인했다.

## 2026-09-16 배포 및 RLS 적용 기록
...
- 최종 운영 기록 PR #6의 `main` 커밋은 `a2ec66d171dd273e120bd1ffd9365061fe506b83`이고 GitHub Actions 실행 `35042370224`가 성공했다. 문서만 바뀌어 운영 앱 번들은 `assets/index-CFK_30WJ.js`로 동일했다.
...
- 전용 두 계정 RLS 경계 검사에서 다음을 확인했다.
  - 비회원에게 비공개 방·메시지·다른 사용자 프로필이 노출되지 않는다.
  - 비회원 메시지 전송과 읽음 커서 위조가 거부된다.
  - 일반 사용자의 관리자 RPC 호출과 허용되지 않은 Storage 경로 사용이 거부된다.
  - 공유 방은 방 요약에 나타나고 비공개 경계 방은 나타나지 않는다.
- 최초 RLS 검사 실패는 정책 문제가 아니라 검증기가 공유 방 전체 멤버 2행을 `maybeSingle()`로 읽은 오류였다. 계정 ID까지 제한하고 쿼리 오류를 즉시 전파하도록 수정했다.
- RLS 적용 뒤 `room_read_cursors`는 publication과 `REPLICA IDENTITY FULL` 설정이 모두 유지됐지만 Realtime 이벤트가 간헐적으로 누락됐다. publication 등록을 트랜잭션 안에서 제거·재등록한 직후 인증 사용자 UPDATE 이벤트와 최종 순번 `4482` 일치를 다시 확인했다.
- 서버 Realtime 누락만으로 읽음 표시가 무기한 오래되지 않도록, 보이는 방 화면은 읽음 커서만 15초마다 가볍게 재조회하도록 후속 보정을 추가했다. Realtime은 즉시 반영 경로로 유지하고 주기 조회는 수렴 안전망으로만 사용한다.
- 읽음 보정 코드 배포 후 30분 이상 운영 상태를 관찰했다. 1분 간격 연속 확인 23회를 포함해 페이지 응답 실패는 0회였고, 종료 시각은 2026-09-16 10:28:22(KST)였다. 종료 후에도 Pages 200, 익명 방·메시지 비노출, 읽음 커서 순번 `4482`를 다시 확인했다.
...
4. `202609140002_room_summaries.sql`
   ...
   - 구버전 앱의 `read_by` 변경을 로그인 사용자 본인의 커서로 연결하고 커서 UPDATE를 Realtime으로 발행한다.
